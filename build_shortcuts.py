...
def build_scanner():
    ...
    A.append(comment(
        ...
        "scanning happens in Code Scanner. Opening another app would normally "
        "let this shortcut carry on and read a stale clipboard, so the alert in "
        "step 3 holds it until you are back."
    ))
    A.append(comment(
        "Generated by build_shortcuts.py in the brightwheel-checkin repo.\n\n"
    ...
    # Open Code Scanner hands off to another app, so the alert below is what
    # keeps the shortcut waiting; without it the run would continue immediately
    # and read whatever was on the clipboard beforehand.
    A.append(act("com.apple.BarcodeScanner.BarcodeScannerIntent",
                 AppIntentDescriptor={
                     "BundleIdentifier": "com.apple.BarcodeScanner",
                     "Name": "Code Scanner",
                     "TeamIdentifier": "0000000000",
                     "AppIntentIdentifier": "BarcodeScannerIntent",
                 },
                 target="launch"))
    ...
    A.append(act("is.workflow.actions.detect.dictionary", UUID=U_DICT,
                 WFInput=ts(out(U_CLIP, "Clipboard"))))
    ...
    A.append(act("is.workflow.actions.showresult",
                 Text=ts("No secret found in that text.\n\nExpected a block "
                         ...
                         out(U_CLIP, "Clipboard"),
                         "\n\nRun this again, and make sure to choose Copy in "
                         "Code Scanner before pressing OK.")))
    A.append(act("is.workflow.actions.conditional", UUID=next(i),
